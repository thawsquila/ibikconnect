...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-6">
        <div class="flex items-center gap-3 mb-2">
            <a href="{{ route('admin.cdc.news.index') }}" class="text-gray-600 hover:text-gray-900">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                </svg>
            </a>
            <h1 class="text-2xl font-bold text-gray-900">Edit Berita CDC</h1>
        </div>
        <p class="text-gray-600">Update informasi berita</p>
    </div>

    <!-- Form Card -->
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form action="{{ route('admin.cdc.news.update', $news) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="excerpt" id="excerpt" rows="3"
                    ...
                    placeholder="Ringkasan singkat berita (akan ditampilkan di preview)">{{ old('excerpt', $news->excerpt) }}</textarea>
                @error('excerpt')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="bg-gray-50 rounded-lg p-4">
                <div class="grid grid-cols-2 gap-4 text-sm">
                    <div>
                        ...
                        <span class="font-semibold text-gray-900 ml-2">{{ number_format($news->views_count) }}</span>
                    </div>
                    @if($news->published_at)
                    <div>
                        <span class="text-gray-500">Dipublikasikan:</span>
                        <span class="font-semibold text-gray-900 ml-2">{{ $news->published_at->format('d M Y H:i') }}</span>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="flex items-center gap-3 pt-4 border-t">
                ...
                <a href="{{ route('admin.cdc.news.index') }}"
