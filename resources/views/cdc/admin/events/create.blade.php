...
@section('title', 'Tambah Event CDC')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-6">
        <div class="flex items-center gap-3 mb-2">
            ...
            <h1 class="text-2xl font-bold text-gray-900">Tambah Event CDC</h1>
        </div>
        <p class="text-gray-600">Buat event atau workshop baru untuk mahasiswa</p>
    </div>

    <!-- Form Card -->
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form action="{{ route('cdc.admin.events.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <textarea name="requirements" id="requirements" rows="4"
                    ...
                    placeholder="Tuliskan persyaratan untuk mengikuti event...">{{ old('requirements') }}</textarea>
                @error('requirements')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
